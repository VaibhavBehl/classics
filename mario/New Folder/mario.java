...
public class mario extends Applet implements Runnable,KeyListener
{
	...
	public void init()
	{
		...
		obj_wall_no=50;
		obj_wall=new int[w/hm][h];
		for(int i=0;i<w/hm;i++)
			for(int j=0;j<h;j+=32)
			{	
				if(i==w/(hm*3) && j<h/2)obj_wall[i][j]=1;
				if(i==w/(hm*2) && j>h/2)obj_wall[i][j]=1;
			}			
	...
	public void run()
	{
		...
		while(true)
		{
			...
			for(int x=xp+wm;x>xp;x--)
				if(obj_wall[yp+1][x]==1){ground=1;break;}
			if(ground!=1)
				yp=yp+1;
			ground=0;
			if(mp=='z'){}
